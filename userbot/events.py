...
def register(**args):
    ...
    def decorator(func):
        ...
        if AVP2:
            if not disable_edited:
                AVP2.add_event_handler(wrapper, events.MessageEdited(**args))
            AVP2.add_event_handler(wrapper, events.NewMessage(**args))
        if AVP3:
            if not disable_edited:
                AVP3.add_event_handler(wrapper, events.MessageEdited(**args))
            AVP3.add_event_handler(wrapper, events.NewMessage(**args))
        if AVP4:
            if not disable_edited:
                AVP4.add_event_handler(wrapper, events.MessageEdited(**args))
            AVP4.add_event_handler(wrapper, events.NewMessage(**args))
        if AVP5:
            if not disable_edited:
                AVP5.add_event_handler(wrapper, events.MessageEdited(**args))
            AVP5.add_event_handler(wrapper, events.NewMessage(**args))
        return wrapper
